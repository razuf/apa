defmodule ApaDiv do
  ...
  @spec bc_div_apa_number({integer(), integer()}, {integer(), integer()}, integer()) :: String.t()
  ...
  def bc_div_apa_number({left_int, left_exp}, {right_int, right_exp}, _scale, rem, _acc)
      when rem == 0 do
    ApaNumber.to_string({div(left_int, right_int), left_exp - right_exp})
  end

  def bc_div_apa_number({left_int, left_exp}, {right_int, right_exp}, scale, _rem, acc)
      when scale == acc do
    ApaNumber.to_string({div(left_int, right_int), left_exp - right_exp})
  end

  def bc_div_apa_number({left_int, left_exp}, {right_int, right_exp}, scale, _rem, acc) do
